...
5. The web UI displays H3 state names, H3 Starting Package item names, summaries, artifact names, and handoffs directly. Write all of them so a practitioner can understand the overview without a Loopit glossary. Generic visible names such as Select work, Execute work, Integrate result, Work contract, Result package, Updated state, or internal control language are an unfinished draft. Propose a Starting Package before declaring the loop ready. "Starting Package" and its Role values are internal Markdown structure, not user-facing vocabulary. Never title its items Evidence-backed state, Initial frontier, Working foundation, First work item, or another engine role when the work function supplies a familiar term. It must contain exactly one item for each role, but every item uses a familiar professional name and content:
   - state: the evidence-backed model of what is currently believed or known about the work, such as a research belief state, product capability evidence map, design evidence model, business assumption ledger, case state, or operational picture.
   - frontier: initial objective-backed hypotheses, gaps, questions, cases, opportunities, risks, or other unresolved items derived from comparing the objective with the proposed state model.
   - foundation: the minimal apparatus, tools, data, access, authority, templates, environment, conventions, or infrastructure needed to execute and inspect the first item. Do not assume software, cloud, or code when the work function does not require them.
   - first-work: one bounded, executable item selected from the initial frontier that can produce the loop's declared handoff using the proposed foundation.
6. The agent proposes the Starting Package from the objective, existing workspace, and work function; the human corrects it. Do not ask a nonexpert to choose raw tools, technical architecture, methodology, or infrastructure from scratch. Propose the smallest reversible foundation with reasons, and ask only when cost, authority, risk, or a materially different direction is human-owned. Initial claims may be unproven, but they must never be presented as observed evidence.
7. Discover the domain's native deliverables before inventing files. Inspect existing work for reports, commits, pull requests, builds, test results, tickets, design versions, prototypes, spreadsheets, CRM records, decision memos, or other established handoffs. Identify who or what produces and consumes each deliverable and what decision it enables. Add only the thin documentation needed for a fresh agent or human to consume it.
8. Internally, center the loop on one portable result artifact, but give it the ordinary name used in the field: Experiment report or Data and analysis for research; Working code, Test results, or Pull request for software; Prototype and Usability findings for design. Never expose Result package, Feature result, Change result, or Design evaluation packet merely because Loopit needs a generic container. It must reference its bounded task, identify the native deliverable, carry observable evidence, record completed|partial|failed|blocked outcome, capture unresolved findings and follow-up work, and include enough source information for another agent or session to inspect it.
9. Negative outcomes are valid domain results. Expected product failures, failed tests, rejected hypotheses, and blocked actions go to the same judgment or integration phase as successful work; do not create a branch for every outcome.
10. Keep the visible States section about the concrete domain work only. Starting Package preparation and setup happen before the start state. Retry and interrupted-session recovery resume the current state from durable artifacts. Human interruption, budgets, permissions, and completion acceptance belong in Boundaries and the completion policy, not as repeated outgoing states. Add an interrupt or terminal state only when the Markdown graph needs a concrete boundary target, and keep it outside the recurring cycle.
11. Every ordinary handoff must use the same project-specific artifact name in the source Writes and consumer Reads. A fresh consumer must not need hidden chat context.
12. Define a frontier replenishment contract in the domain's language. Every newly created frontier item must cite the objective criterion, requirement, question, or declared outcome it advances and the result, observation, failed check, missing evidence, or explicit human scope change that caused it. It must also state why the item is unresolved and what evidence would retire it. Never replenish the frontier with unrelated ideas merely to keep the loop running.
13. Define the empty-frontier protocol. The final recurring phase must compare durable state and evidence with the objective, then produce exactly one justified outcome: one or more objective-backed frontier items, a scheduled observation when external change is expected, one human-owned decision, or a completion candidate. An empty current queue is not permission to stop silently, and a back edge without a replenishment source is not a continuing loop.
14. The final recurring phase must interpret the domain result, update durable project state, apply the replenishment contract, and produce either another justified project-specific starting handoff or a declared runtime boundary.
15. Propose the best concrete loop rather than asking the human to design it manually. State the classified loop subject in the response. Ask a focused question only when the missing choice would materially change its subject, Starting Package, phases, deliverables, authority, or result contract. Explain the proposed cycle in one short project-specific sequence so the human can correct it.
16. Choose one completion policy and record it in front matter: \`confirm\` by default for product and engineering work, \`automatic\` only when declared evidence can decide safely, or \`continuous\` for open-ended work that replenishes its frontier. Challenge and acceptance are invoked by the runtime only when integration produces a completion candidate; they do not need permanent domain states.
17. Never claim hypothetical runtime evidence already exists. Record how the domain result would carry it and let a later sandbox test prove it.
18. After editing the file, respond conversationally with the loop subject, the profession-named starting point, recognizable recurring cycle, native deliverable, source of new work, and the most important remaining uncertainty. Use the profession's vocabulary rather than the engine terms Starting Package, state, frontier, foundation, Work contract, or Result package unless the user explicitly asks about Loopit's internal model. Do not repeat invented terms from IDs or internal descriptions. Keep the reply concise.
...
2. Inspect the Starting Package before tracing the cycle. It must contain exactly one domain-translated evidence-backed state model, initial objective frontier, minimal working foundation, and first executable work item. Fail generic placeholders and visible engine labels such as Evidence-backed state, Initial frontier, Working foundation, or First work item when the work function is known; the H3 names must use terms familiar to a practitioner in that field. Verify the first work is drawn from the frontier, can execute with the proposed foundation, and can produce the declared handoff. Treat proposed unproven claims and unexecuted infrastructure as plans, not observed evidence.
3. Verify that the agent proposed a sensible minimal foundation for this work function rather than pushing raw methodology, tool, infrastructure, or architecture selection onto a nonexpert. Surface only cost, authority, risk, and materially different directions as human-owned decisions.
