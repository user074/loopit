...
3. Infer the user's job function, project type, chosen loop subject, and natural recurring work cycle from their objective and the repository. Different work and different subjects of the same product should have different structures. Use only the domain phases needed for meaningful transformations or handoffs; three to seven recurring states is usually enough, but never force a count.
4. Name every recurring state and handoff in the chosen work mode's native language. When the domain is known, generic visible names such as Select work, Execute work, Integrate result, Work contract, Result package, or Updated state are an unfinished draft. For research, prefer hypotheses, experiment plans, data, reports, verdicts, and belief updates. For app development, prefer capability gaps, feature slices, working builds, sandbox evaluation, product review, and capability-map updates. For operation, prefer the actual recurring operational actions and records. Never mix development and operation into one apparent cycle.
5. Propose a Starting Package before declaring the loop ready. It must contain exactly one item for each role, but every item uses a domain-specific name and content:
...
7. Discover the domain's native deliverables before inventing files. Inspect existing work for reports, commits, pull requests, testable builds, tickets, design versions, prototypes, spreadsheets, CRM records, decision memos, or other established handoffs. Identify who or what produces and consumes each deliverable and what decision it enables. Add only the thin documentation needed for a fresh agent or human to consume it.
8. Center the loop on one portable, domain-named Result package artifact: for example Experiment report, Feature result, Change result, or Design evaluation packet. It must reference its bounded input contract, identify the native deliverable, carry observable evidence, record completed|partial|failed|blocked outcome, capture unresolved findings and candidate follow-up work, and include enough provenance for another agent or session to inspect it.
...
18. After editing the file, respond conversationally with the loop subject, proposed Starting Package, concrete recurring sequence, native deliverable and domain-named Result package, frontier replenishment source, and the most important remaining uncertainty. Keep the reply concise.
...
2. Inspect the Starting Package before tracing the cycle. It must contain exactly one domain-translated evidence-backed state model, initial objective frontier, minimal working foundation, and first executable work item. Fail generic placeholders when the work function is known. Verify the first work is drawn from the frontier, can execute with the proposed foundation, and can produce the declared handoff. Treat proposed unproven claims and unexecuted infrastructure as plans, not observed evidence.
...
5. Check that the proposed loop is translated into the user's domain and chosen work mode. Fail an otherwise generic Select work -> Execute work -> Integrate result loop when the objective or repository supplies enough information to name concrete phases and handoffs. Do not force every domain into the same number of states and do not mix development with operation.
6. Identify the domain's native deliverable and its domain-named Result package. Fail the loop if the result is only chat prose, if its deliverable is unclear, if it is named only Result package despite a known domain, or if a fresh consumer cannot find and inspect it.
