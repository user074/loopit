...
10. Define a frontier replenishment contract in the domain's language. Every newly created frontier item must cite the objective criterion, requirement, question, or declared outcome it advances and the result, observation, failed check, missing evidence, or explicit human scope change that caused it. It must also state why the item is unresolved and what evidence would retire it. Never replenish the frontier with unrelated ideas merely to keep the loop running.
11. Define the empty-frontier protocol. The final recurring phase must compare durable state and evidence with the objective, then produce exactly one justified outcome: one or more objective-backed frontier items, a scheduled observation when external change is expected, one human-owned decision, or a completion candidate. An empty current queue is not permission to stop silently, and a back edge without a replenishment source is not a continuing loop.
12. The final recurring phase must interpret the domain result, update durable project state, apply the replenishment contract, and produce either another justified project-specific starting handoff or a declared runtime boundary.
13. Propose the best concrete loop rather than asking the human to design it manually. State the classified loop subject in the response. Ask a focused question only when the missing choice would materially change its subject, phases, deliverables, authority, or result contract. Explain the proposed cycle in one short project-specific sequence so the human can correct it.
14. Choose one completion policy and record it in front matter: \`confirm\` by default for product and engineering work, \`automatic\` only when declared evidence can decide safely, or \`continuous\` for open-ended work that replenishes its frontier. Challenge and acceptance are invoked by the runtime only when integration produces a completion candidate; they do not need permanent domain states.
15. Never claim hypothetical runtime evidence already exists. Record how the domain result would carry it and let a later sandbox test prove it.
16. After editing the file, respond conversationally with the loop subject, concrete recurring sequence, native deliverable and domain-named Result package, frontier replenishment source, and the most important remaining uncertainty. Keep the reply concise.
...
2. Identify the recurring domain cycle and ignore setup or runtime handlers when describing it. State and frontier -> Work contract -> Result package -> Integrated state and frontier is only the invariant you use to check the cycle; report the actual project-specific phases and artifacts.
3. Check that the proposed loop is translated into the user's domain and chosen work mode. Fail an otherwise generic Select work -> Execute work -> Integrate result loop when the objective or repository supplies enough information to name concrete phases and handoffs. Do not force every domain into the same number of states and do not mix development with operation.
4. Identify the domain's native deliverable and its domain-named Result package. Fail the loop if the result is only chat prose, if its deliverable is unclear, if it is named only Result package despite a known domain, or if a fresh consumer cannot find and inspect it.
5. Trace artifact ownership across one recurrence. Each project-specific producer must write the same named artifact its consumer reads; the domain result must reach judgment or integration; integration must update project state and replenish the frontier.
6. Trace every newly created frontier item back to both an objective criterion and causal evidence. Fail a loop that can invent unrelated work, duplicate resolved work, or continue without stating what evidence would retire the item.
7. Exhaust the current frontier in rehearsal. Verify that the empty-frontier protocol compares state with the objective and produces objective-backed work, scheduled observation, one human decision, or a completion candidate. Fail a loop that stops merely because its initial list is empty or loops back without a source of new work.
8. Test that completed, partial, failed, and blocked work all produce consumable domain results and reach integration without creating outcome-specific branch explosions.
9. Start a hypothetical fresh agent at each handoff using only declared durable artifacts. Check whether it can identify the contract, deliverable, evidence, outcome, provenance, and next responsibility without the construction conversation.
10. Evaluate setup, retries, interrupted-session recovery, human authority, budget, and completion as runtime policies separately. Do not require them to appear as ordinary domain states. Verify only that each policy has a durable resume or stop contract and cannot silently masquerade as successful work.
11. Distinguish structural proof from untested production behavior. Do not pass a claim that requires artifacts or fixtures which do not exist.
12. For completion, verify that integration creates only a candidate and the declared runtime policy performs any required challenge or human acceptance. A challenge does not need to be a permanent graph state.
