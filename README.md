...
- **One-click loop testing** shows one visible route—trace every path, test with a fresh agent, fix or ask the user, then Passed. It repairs agent-owned problems and retests the repaired revision automatically. When the remaining issue requires human intent or authority, the right panel opens a focused review with the question, the agent's recommendation, concrete choices, and a custom-response field. A chosen decision is recorded in `.loopit/loop.md` and the test resumes automatically. Passed means the current loop revision is structurally resumable, has no unresolved construction decision, and specifies later sandbox tests and failure routes; it does not claim that unimplemented production behavior already works. A collapsed audit shows what was checked or changed, while the full Markdown report remains available in `.loopit/test-report.md`.
- **Gated runtime start** appears after structural checks and the separate test section. Start remains locked until the current loop revision passes. Once unlocked, it launches a separate Codex or Claude worker from the declared first task and state, while preserving construction chat as the supervisor surface. Each worker turn has a readable durable record under `.loopit/runs/`. A live Continuous runtime clock counts wall-clock time only while the local worker process remains active; it freezes on pause, failure, interruption, or completion.
