...
- **Runtime control and observability** begin with an integrated project map, followed by State, Next work, and History views. Stable regions come from the user-approved starting frontier and retain the project's own language—a hypothesis, feature, user flow, opportunity, case, or other natural unit. Lower-level runtime tasks and results roll up under those regions instead of constantly rearranging the map. Every progress bar is calculated as resolved tracked units divided by resolved plus open tracked units; its denominator is visible and may grow when evidence reveals legitimate new work. The map has semantic Overview, Trajectory, and Evidence levels: zooming in reveals past result → current activity → next work, then supporting counts, instead of merely enlarging cards. Selecting an area opens a closable dock below the map, so its trajectory and controls never cover neighboring work. The central mission renders an RTS-like Worker → moving action → concrete target scene that changes visibly between planning, reading files, editing, running commands or tests, searching, reporting, and supervisor integration. The active project area echoes the same exact file, command, or action while inactive areas recede. A persistent side panel streams the full normalized operational trail—progress, file changes, commands, outputs, failures, heartbeats, assignment, and loop phase. The same panel can launch an independent read-only observer with both live operational context and durable state, queue steering for the next safe integration, or stop the current worker immediately. The footer summarizes advances, unreviewed results, uncertainty, blocked work, active work, and the latest state transition.
- **Interactive understanding and steering** are attached to the selected project region instead of living in a separate control sidebar. A person can ask what is happening, issue a direction, or open detailed next work without losing map context. The read-only understanding agent answers from state, ledger, reports, and project artifacts with auditable references. Steering is not hidden chat context: it is saved to `.loopit/runtime/STEERING.md`, shown as pending, and applied by the supervisor at the next safe state integration. If the frontier is empty, steering or unattended mode triggers an objective-gap reassessment instead of silently stopping.

The target repository owns readable `.loopit/runtime/STATE.md`, immutable assignments, detailed iteration reports, append-only `LEDGER.md`, `STEERING.md`, and the human review cursor in `REVIEW.md`. A saved result awaiting integration can be integrated after restart; an interrupted assignment remains leased for recovery instead of being forgotten or duplicated. The full design, current contracts, and intentionally deferred work are documented in [Runtime Control Plane](docs/RUNTIME_CONTROL_PLANE.md).
