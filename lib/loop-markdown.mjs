...
function optionalHeadingSection(source, level, title) {
  const marker = "#".repeat(level);
  const heading = new RegExp(`^${marker}\\s+${title}\\s*$`, "mi");
  const match = heading.exec(source);
...
function optionalField(source, label) {
  const match = new RegExp(`^\\*\\*${label}:\\*\\*\\s*(.+)$`, "mi").exec(
    source,
  );
...
export function parseLoopMarkdown(source) {
  ...
  const startingPackageSection = optionalHeadingSection(
    source,
    2,
    "Starting Package",
  );
  const startingPackage = startingPackageSection
    ? entrySections(startingPackageSection, 3).map(({ title: name, body }) => {
        ...
        return {
          ...
          role: oneOf(
            field(body, "Role"),
            ["state", "frontier", "foundation", "first-work"],
            `Starting package ${id} role`,
          ),
          description: field(body, "Description"),
          initialContents: listSection(body, "Initial Contents"),
        };
      })
    : [];

  const canonicalArtifactsSection = optionalHeadingSection(
    source,
    2,
    "Artifacts",
  );
  const combinedArtifactsAndBoundariesSection = optionalHeadingSection(
    source,
    2,
    "Artifacts and Boundaries",
  );
  const artifactsSection = requiredMatch(
    canonicalArtifactsSection ?? combinedArtifactsAndBoundariesSection,
    "Missing Markdown section: ## Artifacts",
  );
  const boundaryKinds = ["interrupt", "complete", "budget"];
  const artifacts = entrySections(artifactsSection, 3)
    .filter(
      ({ body }) =>
        canonicalArtifactsSection ||
        !boundaryKinds.includes(optionalField(body, "Kind")),
    )
    .map(({ title: name, body }) => ({
      id: field(body, "ID"),
      name,
  ...
  const canonicalBoundariesSection = optionalHeadingSection(
    source,
    2,
    "Boundaries",
  );
  const boundariesSection = requiredMatch(
    canonicalBoundariesSection ?? combinedArtifactsAndBoundariesSection,
    "Missing Markdown section: ## Boundaries",
  );
  const boundaries = entrySections(boundariesSection, 3)
    .filter(
      ({ body }) =>
        canonicalBoundariesSection ||
        boundaryKinds.includes(optionalField(body, "Kind")),
    )
    .map(({ title: name, body }) => {
    const id = field(body, "ID");
  ...
  return {
    ...
    completionPolicy: oneOf(
      meta["completion-policy"] || "confirm",
      ["confirm", "automatic", "continuous"],
      "Front matter completion-policy",
    ),
...
${bulletList(item.initialContents)}`,
    )
    .join("\n\n");
...
completion-policy: ${oneLine(loop.completionPolicy || "confirm")}
